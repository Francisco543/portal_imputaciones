...
const APP_BUILD_VERSION = "2026-03-10-recovery-codeid";
...
function extractProjectTypeInfo(projectRow) {
  ...
  const expanded =
    projectRow.exc_ProyectosNav || projectRow.exc_proyectosnav || null;

  const rawCode =
    projectRow.exc_projecttypecode ?? expanded?.exc_projecttypecode ?? null;

  const formatted =
    projectRow[
      "exc_projecttypecode@OData.Community.Display.V1.FormattedValue"
    ] ||
    expanded?.[
      "exc_projecttypecode@OData.Community.Display.V1.FormattedValue"
    ] ||
...
function getWorkItemModeByProject(projectId) {
  ...
  return (
    projectWorkItemModeById[projectId] || projectsMap[projectId]?.mode || "none"
  );
}

function getAvailableWorkItemsByProject(projectId) {
  if (!projectId) return [];
  return Array.isArray(workItemsByProject[projectId])
...
function isWorkItemRequiredByProject(projectId) {
  return (
    getWorkItemModeByProject(projectId) !== "none" &&
    hasAvailableWorkItemsByProject(projectId)
  );
}
...
function createWorkItemFields(mode, name) {
  ...
  const meta =
    arguments[2] && typeof arguments[2] === "object" ? arguments[2] : null;
...
function normalizeEntryWorkItem(entry) {
  ...
  const inferredMode =
    entry.workItemMode === "task" || entry.workItemMode === "request"
      ? entry.workItemMode
      : entry.request || entry.requestId
        ? "request"
        : entry.task || entry.taskId
          ? "task"
          : "none";
  ...
  const inferredAssignedHoursRaw =
    entry.workItemAssignedHours ?? entry.assignedHours ?? null;
  const inferredAssignedHours =
    inferredAssignedHoursRaw === null ||
    inferredAssignedHoursRaw === undefined ||
    inferredAssignedHoursRaw === ""
      ? NaN
      : Number(inferredAssignedHoursRaw);

  const normalized = createWorkItemFields(inferredMode, inferredName, {
    id: inferredId,
    assignedHours: Number.isFinite(inferredAssignedHours)
      ? inferredAssignedHours
      : null,
  });
...
function buildWorkItemDisplayName(row, mode) {
  ...
  if (code && description) {
    const sameValue =
      code.trim().toLowerCase() === description.trim().toLowerCase();
...
function pickWorkItemId(row, mode) {
  ...
  const idFields =
    mode === "request"
      ...
      : [
          "exc_tareasconsultoriaid",
          "_exc_tareasconsultoria_value",
          "cr774_tareaid",
        ];
...
function inferPersistedWorkItemInfo(row, fallbackMode = "none") {
  ...
  const safeFallback =
    fallbackMode === "task" || fallbackMode === "request"
      ? fallbackMode
      : "none";
...
function normalizeWorkItemOption(option, fallbackMode = "none") {
  ...
  if (typeof option === "object") {
    ...
    const mode =
      option.mode === "task" || option.mode === "request"
        ? option.mode
        : fallbackMode;
...
async function fetchWorkItemsForProject(projectContext, mode) {
  ...
  const primaryUrl = `/_api/${cfg.entitySet}?$top=200&$filter=${cfg.filterColumn} eq ${filterValueEscaped}`;
  ...
  try {
    ...
    if (cfg.fallbackEntitySet && cfg.fallbackFilterColumn) {
      ...
      try {
        ...
        const fallbackValues = Array.isArray(fallbackData?.value)
          ? fallbackData.value
          : [];
    ...
    if (mode === "request" && codeIdEscaped) {
      try {
        ...
        console.error(
          `[work-items][${mode}] Error intento por código`,
          codeError,
        );
      }
    }
...
function canEnableAddButton() {
  ...
  const okTask = !taskRequired || !!selectedTask;
...
function preloadEntries(imputaciones) {
  // CAMBIO DOCUMENTADO:
  ...
  imputaciones.forEach((item) => {
    if (!item.exc_Proyectoimputacion) {
      console.warn(
        "Imputación sin proyecto, se ignorará:",
        item.exc_diarioimputacionid,
      );
      return;
    }

    const projectCode =
      item.exc_Proyectoimputacion.exc_iteminternalid?.split("-")[0] || "-";
    const projectName =
      item.exc_Proyectoimputacion.exc_proyectodescripcion || "-";
    ...
    const projectId =
      item._exc_proyectoimputacion_value ||
    ...
    const persistedWorkItemInfo = inferPersistedWorkItemInfo(
      item,
      projectedMode,
    );
    ...
    const key =
      journalBatchName && lineNumber !== null
    ...
    if (!grouped[key]) {
      grouped[key] = {
        ...
        lineNumber,
        journalBatchName,
        headerId: item._exc_cr774_registro_value || currentHeaderId || null,
    ...
    grouped[key].weekHoursArray[DAY_TO_INDEX[dayKey]] =
      hours > 0 ? hours : null;
  ...
  entries = Object.values(grouped).map((entry) => {
    entry.dbRecordIds = Array.from(
      new Set((entry.dbRecordIds || []).filter(Boolean)),
    );
  ...
  const imputacionesSinProyecto = imputaciones.filter(
    (i) => !i.exc_Proyectoimputacion,
  );

  debugLog("📊 Entries agrupadas:", entries);
  debugLog(
    "❌ Imputaciones sin proyecto (ignoradas):",
    imputacionesSinProyecto.length,
  );
...
function loadState() {
  try {
    ...
    if (parsed && Array.isArray(parsed.detailedLog))
      detailedLog = parsed.detailedLog;
...
function getSelectedWeekStartDate() {
  ...
  const pickerDate =
    document.getElementById("datePicker")?._flatpickr?.selectedDates?.[0] ||
    null;
...
function buildStableEntryId(entry) {
  ...
  return [
    "header",
    encodeStableKeyPart(
      normalized.headerId || currentHeaderId || normalized.weekStart || "",
    ),
    "project",
...
function syncWeekHoursArray(entry) {
  if (!entry) return;
  if (
    !Array.isArray(entry.weekHoursArray) ||
    entry.weekHoursArray.length !== 5
  ) {
...
function stagePendingEntryPersistence(entry, prevDraft, nextDraft) {
  ...
  entry._pendingPersistence = {
    ...
    hadPersistedRecordsBefore:
      Array.isArray(entry.dbRecordIds) &&
      entry.dbRecordIds.filter(Boolean).length > 0,
...
const HourValidators = {
  ...
  validateDailyTotals(entries, entryId, draft) {
    ...
    for (const day of DAYS_ARRAY) {
      ...
      if (total > cap + 1e-9) {
        const disponible = Math.max(0, cap - totalsExcl[day]);
        return {
          valid: false,
          error: `Total ${DAY_NAMES_LOWER[day]} supera ${cap}h. Disponible: ${disponible.toFixed(1)}h.`,
          dayWithError: day,
        };
      }
    }
  ...
  parseAndValidateHourInput(raw) {
    ...
    if (value > 8) {
      return { valid: false, error: "Las horas no pueden superar 8" };
    }
...
async function ensureEntryAssignedHoursLimit(entry) {
  ...
  if (
    ...
    !normalized?.workItemMode ||
    normalized.workItemMode === "none"
  ) {
    return null;
  }

  const entitySet =
    normalized.workItemMode === "task"
      ? "exc_tareasconsultorias"
      : "exc_peticioneses";

  try {
    const record = await fetchJson(
      `/_api/${entitySet}(${normalized.workItemId})?$select=exc_assignedhours`,
    );
    ...
    console.warn(
      "[assigned-hours] No se pudo recuperar exc_assignedhours, se omite la validacion.",
      {
...
async function validateAssignedHoursLimit(entry, draft) {
  ...
  const hasSelectedWorkItem =
    (normalized.workItemMode === "task" ||
      normalized.workItemMode === "request") &&
    !!String(normalized.workItemId || "").trim() &&
    !!String(
      normalized.workItemName || normalized.task || normalized.request || "",
    ).trim();
  ...
  if (normalized.workItemMode === "task" && normalized.workItemId) {
    filterParts.push(
      `_exc_tareasconsultoria_value eq '${normalized.workItemId}'`,
    );
  }
  ...
  const currentRecordIds = new Set(
    Object.values(normalized.dayRecordIds || {}).filter(Boolean),
  );
  ...
  const totalAfterSave =
    persistedTotal - currentEntryPersistedTotal + nextDraftTotal;
...
function shouldShowTaskAvailabilityForEntry(entry) {
  ...
  return (
    normalized?.workItemMode === "task" &&
    !!String(normalized?.workItemName || "").trim()
  );
}
...
function renderTaskAvailabilityCellContent(entry) {
  if (!shouldShowTaskAvailabilityForEntry(entry)) {
    return '<p class="small text-secondary fst-italic mb-0">-</p>';
  }
...
async function ensureTaskAvailabilityForEntry(entry) {
  ...
  try {
    ...
    const data = await fetchJson(url);
    const values = Array.isArray(data?.value) ? data.value : [];
    const totalImputedHours = values.reduce(
      (sum, item) => sum + Number(item?.exc_quantity || 0),
      0,
    );
    ...
    console.warn(
      "[task-availability] No se pudo calcular la disponibilidad de la tarea.",
      {
        key,
        error,
      },
...
async function refreshTaskAvailabilityForVisibleEntries() {
  const taskEntries = entries.filter((entry) =>
    shouldShowTaskAvailabilityForEntry(entry),
  );
  ...
  await Promise.all(
    Array.from(uniqueEntriesByKey.values()).map((entry) =>
      ensureTaskAvailabilityForEntry(entry),
    ),
  );
...
const EntryManager = {
  createEntry(
    ...
    hasDaily,
    daily,
    selectedWorkItemOption = null,
  ) {
    const hours = this._createHoursObject(hasDaily, daily);
    const workItemMode = taskRequired
      ? getWorkItemModeByProject(projectId)
      : "none";
    const option = normalizeWorkItemOption(
      selectedWorkItemOption,
      workItemMode,
    );
    const workItemFields = createWorkItemFields(workItemMode, selectedTask, {
      id: option?.id || null,
...
function initFlatpickrInclusive() {
  ...
  if (!el || typeof flatpickr === "undefined" || (!startDate && cabeceraId))
    return;
...
function markNonMondays(_dObj, _dStr, _fp, dayElem) {
  try {
...
function formatPeriodES(start, end) {
  const fmtDayMonth = new Intl.DateTimeFormat("es-ES", {
    day: "2-digit",
    month: "short",
  });
...
async function loadWorkItemsContextForProject(projectId, projectRaw) {
  ...
  const items = await fetchWorkItemsForProject(
    { navId, codeId, projectCode },
    mode,
  );
...
function initProjectAutocomplete() {
  ...
  async function fetchProjectsPage({ term, nextLink }) {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Error al cargar proyectos (${res.status}): ${txt || res.statusText}`,
      );
    }
  ...
  function normalizeProjectRow(p) {
    const navId =
      p?._exc_proyectosnav_value ||
      p?.exc_ProyectosNav?.exc_proyectosnavid ||
      "";
    const projectId = p?.exc_proyectoimputacionid || navId || "";
    const code = (p?.exc_iteminternalid ?? p?.exc_proyecto ?? "")
      .toString()
      .trim()
      .split("-")[0];
  ...
  function renderDropdown() {
    ...
    const loadingHtml = isLoading
      ...
      : currentNextLink
        ? `<div class="px-3 py-2 small text-secondary border-top bg-light">Scroll para cargar más…</div>`
        : "";
  ...
  async function loadFirstPage(term) {
    ...
    try {
      ...
      items = dedupeProjects(
        page.values.map(normalizeProjectRow).filter((x) => x.id),
      );
  ...
  async function loadNextPage() {
    ...
    try {
      ...
      const more = page.values.map(normalizeProjectRow).filter((x) => x.id);
...
function initTaskDropdown() {
  ...
  function getCurrentMode() {
    ...
    return (
      projectWorkItemModeById[selectedProjectId] ||
      projectsMap[selectedProjectId]?.mode ||
      "none"
    );
  }
  ...
  function filterTasks(tasks, query) {
    ...
    return tasks.filter((t) =>
      getWorkItemOptionText(t, getCurrentMode()).toLowerCase().includes(q),
    );
  }
  ...
  if (btnAll) {
    btnAll.addEventListener("click", (e) => {
      ...
      if (all.length === 0) {
        ...
        showToast?.(
          mode === "request"
            ? "Este proyecto no tiene peticiones."
...
async function handleAddHours() {
  ...
  const taskKey = taskRequired ? String(selectedTask || "").trim() : "";
  const taskId = taskRequired
    ? String(selectedTaskOption?.id || "").trim()
    : "";

  const hoursRaw = document.getElementById("hoursInput")?.value ?? "";
  let daily =
    typeof parseHoursInput === "function"
      ? parseHoursInput(hoursRaw)
      : parseFloat(String(hoursRaw).trim().replace(",", "."));
  ...
  const existingEntryIndex = EntryManager.findExistingEntry(
    projectId,
    taskRequired,
    taskKey,
    taskId,
...
async function createDataverseRecord(entitySet, payload, idCandidates = []) {
  ...
  if (!res.ok) {
    throw new Error(
      `No se pudo crear en ${entitySet}. HTTP ${res.status}. ${rawBody}`,
    );
  }
...
async function updateDataverseRecord(entitySet, recordId, payload) {
  ...
  if (!res.ok && res.status !== 204) {
    const details = await res.text().catch(() => "");
    throw new Error(
      `No se pudo actualizar ${entitySet}(${recordId}). HTTP ${res.status}. ${details}`,
    );
  }
}
...
async function deleteDataverseHeaderById(recordId) {
  ...
  if (!res.ok && res.status !== 204 && res.status !== 404) {
    const details = await res.text().catch(() => "");
    throw new Error(
      `No se pudo eliminar la cabecera ${recordId}. HTTP ${res.status}. ${details}`,
    );
  }
}
...
function buildHeaderPayload() {
  ...
  if (!start)
    throw new Error("No hay fecha seleccionada para crear la cabecera.");
...
async function ensureCurrentHeader() {
  ...
  if (!id) {
    throw new Error(
      "Dataverse creó la cabecera pero no devolvió su identificador.",
    );
  }
...
function appendDailyWorkItemBindings(payload, entry) {
  ...
  if (normalized.workItemMode === "task") {
    payload["exc_TareasConsultoria@odata.bind"] =
      `/exc_tareasconsultorias(${workItemId})`;
  }
...
async function createDailyRecord(entry, day, hours) {
  ...
  const { id } = await createDataverseRecord("exc_diarioimputacions", payload, [
    "exc_diarioimputacionid",
  ]);

  if (!id) {
    throw new Error(
      `No se recibió ID al crear el diario del ${DAY_LABELS[day]}.`,
    );
  }
...
async function deleteDataverseRecordById(recordId) {
  ...
  if (!ENABLE_DB_DELETE) {
    debugLog(
      "[DEBUG][NO-DB] DELETE simulado en exc_diarioimputacions:",
      recordId,
    );
  ...
  if (!res.ok && res.status !== 204 && res.status !== 404) {
    const details = await res.text().catch(() => "");
    throw new Error(
      `No se pudo eliminar ${recordId}. HTTP ${res.status}. ${details}`,
    );
  }
}
...
function syncEntryRecordIds(entry) {
  ...
  entry.dbRecordIds = Array.from(
    new Set(DAYS_ARRAY.map((day) => dayRecordIds[day]).filter(Boolean)),
  );
}
...
async function deleteZeroHourDayRecords(entry, prevDraft, nextDraft) {
  ...
  const recordsToDelete = DAYS_ARRAY.map((day) => {
  ...
  debugLog(
    "[edit-hours] registros diarios a borrar por pasar a 0h:",
    recordsToDelete,
  );
  await deleteDataverseRecordsByIds(
    recordsToDelete.map(({ recordId }) => recordId),
  );
...
function renderTable() {
  ...
  tbody.innerHTML = entries.length
    ? entries.map((entry) => renderTableRow(entry)).join("")
    : "";
  ...
  refreshTaskAvailabilityForVisibleEntries().catch((error) => {
    console.warn(
      "[task-availability] No se pudieron refrescar las horas disponibles.",
      error,
    );
  });
  refreshIcons();
...
function renderTableRow(entry) {
  ...
  const totalWeek = calculateWeeklyTotal(entry);
    ...
    <td class="p-3 text-center ip20-col-task">
      ${
        taskText
          ...
          : '<p class="small text-secondary fst-italic mb-0">-</p>'
      }
    </td>
...
function hasObservations(entry) {
  return Object.values(entry.observaciones).some(
    (txt) => (txt || "").trim() !== "",
  );
}

function calculateWeeklyTotal(entry) {
  return DAYS_ARRAY.reduce((sum, day) => sum + (entry.hours[day] || 0), 0);
}
...
function renderEditInput(entryId, day, currentValue) {
  const v =
    hoursDraft && editingHoursEntryId === entryId
      ? hoursDraft[day]
      : currentValue || 0;
...
window.cancelEditHours = async function () {
  ...
  if (entries.length === 0) {
    try {
      ...
      console.error(
        "No se pudo limpiar la cabecera vacía al cancelar la edición:",
        error,
      );
...
window.confirmEditHours = async function () {
  ...
  const hadPersistedRecordsBefore =
    Array.isArray(entry.dbRecordIds) &&
    entry.dbRecordIds.filter(Boolean).length > 0;
  ...
  const totalsValidation = HourValidators.validateDailyTotals(
    entries,
    entry.id,
    draft,
  );
  ...
  if (!hasAnyHours) {
    try {
      ...
      console.error(
        "[edit-hours] Error eliminando registros diarios puestos a 0h",
        error,
      );
  ...
  try {
    const assignedHoursValidation = await validateAssignedHoursLimit(
      entry,
      draft,
    );
    ...
    showToast(
      "No se pudo validar el número máximo de horas asignadas.",
      "error",
    );
    return;
  }
...
window.openModal = function (entryId) {
  ...
  const DIAS_CON_HORAS = DAYS_ARRAY.map((day) => ({
    key: day,
  ...
  if (DIAS_CON_HORAS.length === 0) {
    ...
    DIAS_CON_HORAS.forEach((d) =>
      renderObservationDay(container, d, entry, false),
    );
  }
...
function renderObservationDay(container, dayInfo, entry, readOnly) {
  ...
  const extraNoCommentClass =
    !readOnly && !hasComment ? " border-secondary" : "";
...
window.saveObservation = async function () {
  ...
  const daysWithHours = DAYS_ARRAY.filter(
    (day) => (entry.hours?.[day] || 0) > 0,
  );
  ...
  if (missingComments.length > 0) {
    ...
    showToast(
      `Los siguientes días requieren comentarios: ${daysList}`,
      "error",
    );
  ...
  try {
    if (pendingPersistence) {
      ...
      await deleteZeroHourDayRecords(
        entry,
        pendingPersistence.prevDraft,
        pendingPersistence.nextDraft,
      );
...
window.closeModal = async function (preserveChanges = false) {
  ...
  if (entry && pendingPersistence && !preserveChanges) {
    const hadPersistedRecordsBefore =
      !!pendingPersistence.hadPersistedRecordsBefore;
...
function updateTotalPanel() {
  ...
  entries.forEach((entry) => {
    DAYS_ARRAY.forEach((day) => {
      currentHours[day] += Number(entry.hours?.[day] || 0);
    });
  });
  ...
  if (totalWeek > WEEK_LIMIT) {
    ...
    diffText.textContent = `Te quedan ${diff}h disponibles`;
    diffText.className = "small mt-1 mb-0 text-dark fw-semibold";
    if (alertIcon) hideEl(alertIcon);
  }
}
...
window.trackCommentChanges = function (entryId) {
  ...
  if (hasChanges && allFilled) {
    ...
    setThemedButtonState(saveBtn, false);
  }
};
...
window.confirmDelete = async function () {
  ...
  const previousText = confirmBtn?.innerHTML || "";
  ...
  try {
    ...
    if (confirmBtn) {
      ...
      confirmBtn.innerHTML = previousText;
    }
  }
};
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  debugLog(
    "[Imputacion20] build",
    APP_BUILD_VERSION,
    WORK_ITEM_QUERY_CONFIG?.request,
  );
  const params = new URLSearchParams(window.location.search);
  const cabeceraId = params.get("id");
  ...
  if (cabeceraId) {
    try {
      const [imputaciones, cabecera] = await Promise.all([
        fetchJson(
          `/_api/exc_diarioimputacions?$filter=_exc_cr774_registro_value eq ${cabeceraId}&$expand=exc_Proyectoimputacion`,
        ),
      ...
      const fechaInicioRaw =
        cabecera[
          "cr774_fechaderegistro@OData.Community.Display.V1.FormattedValue"
        ];
